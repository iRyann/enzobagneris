  ...
  if (!post) {
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-nature-light text-center px-4">
            ...
            <Link to="/blog" className="text-nature-accent hover:underline">Retour au journal</Link>
        </div>
    );
  }
    ...
    <article className="bg-nature-light min-h-screen pt-24 pb-20 animate-in fade-in duration-500">
      ...
      <div className="w-full h-[50vh] relative mb-12 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-12 bg-gradient-to-t from-black/80 to-transparent">
            <div className="container mx-auto">
                <Link to="/blog" className="inline-flex items-center text-white/80 hover:text-white mb-6 text-sm font-display tracking-widest transition-colors">
                ...
                <div className="flex flex-wrap items-center gap-6 text-white/90 font-serif text-sm">
                     ...
                     <span className="bg-nature-accent px-3 py-1 text-xs font-bold rounded uppercase tracking-wider">{post.category}</span>
                </div>
            </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 max-w-3xl">
        ...
        <div className="flex gap-2 mb-8 flex-wrap">
            {post.tags.map(tag => (
                <span key={tag} className="flex items-center gap-1 text-xs text-nature-muted bg-nature-dark/5 px-3 py-1 rounded-full">
                    <Tag size={12} /> {tag}
                </span>
            ))}
        </div>
